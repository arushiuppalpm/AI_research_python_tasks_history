...
def calendar_daily_177_01(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 177.1 created on 2026-03-14."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_177_02(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 177.2 created on 2026-03-14."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_177_03(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 177.3 created on 2026-03-14."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_177_04(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 177.4 created on 2026-03-14."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_177_05(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 177.5 created on 2026-03-14."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_177_06(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 177.6 created on 2026-03-14."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_177_07(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 177.7 created on 2026-03-14."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_187_01(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 187.1 created on 2026-03-24."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_187_02(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 187.2 created on 2026-03-24."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_187_03(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 187.3 created on 2026-03-24."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_187_04(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 187.4 created on 2026-03-24."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_187_05(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 187.5 created on 2026-03-24."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_187_06(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 187.6 created on 2026-03-24."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_187_07(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 187.7 created on 2026-03-24."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_197_01(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 197.1 created on 2026-04-03."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_197_02(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 197.2 created on 2026-04-03."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_197_03(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 197.3 created on 2026-04-03."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_197_04(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 197.4 created on 2026-04-03."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_197_05(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 197.5 created on 2026-04-03."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_197_06(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 197.6 created on 2026-04-03."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_197_07(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 197.7 created on 2026-04-03."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_207_01(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 207.1 created on 2026-04-13."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_207_02(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 207.2 created on 2026-04-13."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_207_03(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 207.3 created on 2026-04-13."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_207_04(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 207.4 created on 2026-04-13."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_207_05(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 207.5 created on 2026-04-13."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_207_06(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 207.6 created on 2026-04-13."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_207_07(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 207.7 created on 2026-04-13."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_217_01(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 217.1 created on 2026-04-23."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_217_02(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 217.2 created on 2026-04-23."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_217_03(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 217.3 created on 2026-04-23."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_217_04(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 217.4 created on 2026-04-23."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_217_05(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 217.5 created on 2026-04-23."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_217_06(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 217.6 created on 2026-04-23."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_217_07(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 217.7 created on 2026-04-23."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_227_01(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 227.1 created on 2026-05-03."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_227_02(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 227.2 created on 2026-05-03."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_227_03(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 227.3 created on 2026-05-03."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_227_04(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 227.4 created on 2026-05-03."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_227_05(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 227.5 created on 2026-05-03."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_227_06(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 227.6 created on 2026-05-03."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_227_07(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 227.7 created on 2026-05-03."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_237_01(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 237.1 created on 2026-05-13."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_237_02(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 237.2 created on 2026-05-13."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_237_03(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 237.3 created on 2026-05-13."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_237_04(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 237.4 created on 2026-05-13."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_237_05(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 237.5 created on 2026-05-13."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_237_06(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 237.6 created on 2026-05-13."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_237_07(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 237.7 created on 2026-05-13."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_247_01(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 247.1 created on 2026-05-23."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_247_02(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 247.2 created on 2026-05-23."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_247_03(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 247.3 created on 2026-05-23."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_247_04(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 247.4 created on 2026-05-23."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_247_05(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 247.5 created on 2026-05-23."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_247_06(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 247.6 created on 2026-05-23."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def calendar_daily_247_07(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 247.7 created on 2026-05-23."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]

...
def reports_daily_192_01(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 192.1 created on 2026-03-29."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_192_02(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 192.2 created on 2026-03-29."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_192_03(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 192.3 created on 2026-03-29."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_192_04(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 192.4 created on 2026-03-29."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_192_05(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 192.5 created on 2026-03-29."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_192_06(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 192.6 created on 2026-03-29."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_192_07(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 192.7 created on 2026-03-29."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_202_01(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 202.1 created on 2026-04-08."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_202_02(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 202.2 created on 2026-04-08."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_202_03(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 202.3 created on 2026-04-08."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_202_04(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 202.4 created on 2026-04-08."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_202_05(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 202.5 created on 2026-04-08."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_202_06(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 202.6 created on 2026-04-08."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_202_07(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 202.7 created on 2026-04-08."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_212_01(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 212.1 created on 2026-04-18."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_212_02(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 212.2 created on 2026-04-18."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_212_03(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 212.3 created on 2026-04-18."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_212_04(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 212.4 created on 2026-04-18."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_212_05(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 212.5 created on 2026-04-18."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_212_06(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 212.6 created on 2026-04-18."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_212_07(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 212.7 created on 2026-04-18."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_222_01(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 222.1 created on 2026-04-28."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_222_02(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 222.2 created on 2026-04-28."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_222_03(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 222.3 created on 2026-04-28."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_222_04(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 222.4 created on 2026-04-28."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_222_05(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 222.5 created on 2026-04-28."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_222_06(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 222.6 created on 2026-04-28."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_222_07(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 222.7 created on 2026-04-28."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_232_01(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 232.1 created on 2026-05-08."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_232_02(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 232.2 created on 2026-05-08."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_232_03(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 232.3 created on 2026-05-08."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_232_04(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 232.4 created on 2026-05-08."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_232_05(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 232.5 created on 2026-05-08."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_232_06(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 232.6 created on 2026-05-08."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_232_07(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 232.7 created on 2026-05-08."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_242_01(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 242.1 created on 2026-05-18."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_242_02(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 242.2 created on 2026-05-18."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_242_03(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 242.3 created on 2026-05-18."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_242_04(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 242.4 created on 2026-05-18."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_242_05(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 242.5 created on 2026-05-18."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_242_06(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 242.6 created on 2026-05-18."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_242_07(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 242.7 created on 2026-05-18."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_252_01(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 252.1 created on 2026-05-28."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_252_02(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 252.2 created on 2026-05-28."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_252_03(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 252.3 created on 2026-05-28."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_252_04(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 252.4 created on 2026-05-28."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_252_05(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 252.5 created on 2026-05-28."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_252_06(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 252.6 created on 2026-05-28."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
...
def reports_daily_252_07(tasks: list[Task], reference: date | None = None) -> dict[str, object]:
    """Daily taskflow feature 252.7 created on 2026-05-28."""
    today = reference or date.today()
    active = [task for task in tasks if task.status != Status.DONE]
    completed = [task for task in tasks if task.status == Status.DONE]
    blocked = [task for task in tasks if task.status == Status.BLOCKED]
